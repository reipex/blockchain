 ...
 * -Certificate
 * -Difficulty
 * -Winning nonce to remove in next versions
 * -Transaction list
 */
public class Block {
     ...
     */
    public Block(long timestamp, int blockNum, String previousBlockHash, Certificate certificate,
                 long difficulty, int winningNonce, String ledgerHash, List<String> transactions,
                 String minerSignature, int minerSignatureIndex) {
        ...
        try {
            ...
            for (int i = 0; i < transactions.size(); i++) {
                if (transactions.get(i).length() > 10) {
                    transactionsString.append(transactions.get(i)).append("*");
                }
            }
            MessageDigest md = MessageDigest.getInstance("SHA-256");
            transactionsString.substring(0, transactionsString.length() - 1);
    ...
    public Block(String rawBlock) {
        ...
        try {
            ...
            StringBuilder transactionsString = new StringBuilder(parts[2].replace("{", "").replace("}", ""));
            this.transactions = new ArrayList<>();
            String[] rawTransactions = transactionsString.toString().split("\\*"); //Transactions are separated by an asterisk, as the colon, double-colon, and comma are all used in other places, and would be a pain to use here.

            this.transactions.addAll(Arrays.asList(rawTransactions));
             ...
             */
            try {
                transactionsString = new StringBuilder();
                //Transaction format: FromAddress;InputAmount;ToAddress1;Output1;ToAddress2;Output2... etc.
                for (int i = 0; i < transactions.size(); i++) {
                ...
                {
                    transactionsString = new StringBuilder(transactionsString.substring(0, transactionsString.length() - 1));
                }
                  ...
                  */
                    try {
                        ...
                        for (int j = 0; j < transactionParts.length - 2; j++) {
                            transactionData.append(transactionParts[j]).append(";");
                        }
                        transactionData = new StringBuilder(transactionData.substring(0, transactionData.length() - 1));
                        if (!MerkleAddressUtility.verifyMerkleSignature(transactionData.toString(), transactionParts[transactionParts.length - 2], transactionParts[0], Long.parseLong(transactionParts[transactionParts.length - 1]))) {
    ...
    public String getRawBlock() {
        ...
        for (int i = 0; i < transactions.size(); i++) {
            if (transactions.get(i).length() > 10) {
                transactionString.append(transactions.get(i)).append("*");
            }
        }
        ...
        {
            transactionString = new StringBuilder(transactionString.substring(0, transactionString.length() - 1));
        }
    ...
    public long getMinerSignatureIndex() {
        return minerSignatureIndex;
    }
}
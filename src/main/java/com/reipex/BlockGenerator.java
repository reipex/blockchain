 ...
 * In order for a block to be valid on the network, it must include a valid timestamp, blockNum, previousBlockHash, difficulty, winningNonce, ledgerHash (not in current version), certificate, and be signed by the miner.
 * <p>
 * When Reipex receives a certificate over RMI, it will gather the pool of pending transactions, test each transaction for available balances and such, and attempt to create a block, and send it to the network.
 * Block Format:
...
public class BlockGenerator {
    /**
     * The parameters are a mouthful. This method takes all of the building blocks block, and compiles them together into a format that the network will accept.
